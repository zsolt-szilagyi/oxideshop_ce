...
class SmartyContextTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testGetShopCompileDirectory()
    {
        $config = $this->getConfigMock();
        $config->method('getConfigParam')
            ->with('sCompileDir')
            ->will($this->returnValue('shop_tmp'));

        $utilsView = $this->getUtilsViewMock();

        $smartyContext = new SmartyContext($config, $utilsView);
        $this->assertSame('shop_tmp', $smartyContext->getShopCompileDirectory());
    }

    public function testGetCurrentShopId()
    {
        $config = $this->getConfigMock();
        ...
        $smartyContext = new SmartyContext($config, $utilsView);
        $this->assertSame(1, $smartyContext->getCurrentShopId());
    }
