        ...
        foreach ($extendedClasses as $oxidEshopClass => $moduleClasses) {
            foreach ($moduleClasses as $sModulePath) {
                ...
                if ($this->isNamespacedClass($sModulePath)) {
                    //$doctrineClassLoader = \Doctrine\Common\ClassLoader::getClassLoader($sModulePath);
                    $doctrineClassLoader = new \Doctrine\Common\ClassLoader('OxidEsales\\EshopAcceptanceTestModule');
                    //var_dump($doctrineClassLoader->canLoadClass($sModulePath));

                    //$composerClassLoader = new \Composer\Autoload\ClassLoader();

                    //$composerClassLoader = \Composer\Autoload\ClassLoader::getClassLoader($sModulePath);

                    //var_dump($composerClassLoader->findFile('\OxidEsales\EshopCommunity\Core\Module\ModuleList'));
                    //var_dump($composerClassLoader->findFile('ModuleList'));

                    //     $completeExtensionPath = $composerClassLoader->findFile($sModulePath);
                    //var_dump($sModulePath . ' ' . $completeExtensionPath);
